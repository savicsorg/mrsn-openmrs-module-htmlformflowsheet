    ...
    <c:forEach var="enc" items="${model.encounterListForChart}">
        ...
        <c:if test="${found == 'true' || !(empty model.conceptsToShow)}">
            <tr style="height:30px;">
                ...
                <td>
                    <c:if test="${model.readOnly == 'false' && !empty enc.encounterId}">
                        <a href="javascript:void(0)" onClick="resizeIFrame${model.portletUUID}(${model.windowHeight});
                                                                        showEncounterPopup('${model.portletUUID}', ${enc.encounterId},${model.formId},${model.showHtmlFormInstead},${model.showProvider}, '${model.providerHeader }', '${model.conceptsToShow}')">
                ...
                <script type="text/javascript">
                    $j('#div_${enc.encounterId}_${model.portletUUID}').load(openmrsContextPath + '/module/htmlformentry/htmlFormEntry.form?inPopup=true&encounterId=${enc.encounterId}&formId=${model.formId}&mode=VIEW htmlform', function () {
                        resizeHtmlFormIframe('${model.formId}', '${model.portletUUID}'); });
                </script>
            </c:if>
        </tr>
    </c:if>
